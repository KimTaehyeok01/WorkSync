...
| 이름 | 역할 | 담당 도메인 |
|------|------|-------------|
| **김태혁** | Backend | Spring Security · JWT 인증 필터 · WebSocket 설정 · 전자결재 · 채팅 · 사원/부서 관리 · 알림 · 감사로그 · 파일 · CI/CD 구성 |
| **김민준** | Backend | 게시판 · 업무(Task) · 연차/휴가 |
| **유재혁** | Backend | 부서 · 근태(출퇴근) · 대시보드 집계 · 파일 업로드 |
| **정소희** | Frontend | 공통 컴포넌트 설계 · 로그인/인증 · 대시보드 · 채팅 · 알림 · 조직관리 · 업무 |
| **이유리** | Frontend | 전자결재 · 게시판 · 업무(Task) · 메신저 · 조직도 화면 |